...
const getActiveViewportElement = () => {
    ...
    if (viewportIndex >= 0) {
        activeViewport = $('.imageViewerViewport').get(viewportIndex);
    }
...
const getEnabledElementForActiveElement = () => {
    ...
    return getEnabledElement(activeViewportElement);
};
...
const toggleCinePlay = element => {
    // Get the active viewport element if none is passed as parameter
    element = element || getActiveViewportElement();
...
const switchToImageByIndex = newImageIdIndex => {
    var element = getActiveViewportElement();
    cornerstoneTools.scrollToIndex(element, newImageIdIndex);
};

const switchToImageRelative = distanceFromCurrentIndex => {
    var element = getActiveViewportElement();
