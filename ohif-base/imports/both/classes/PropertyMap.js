...
function _getPropertyValue(propertySeparator, indexSeparator, targetObject, propertyName) {
    ...
    if (fragmentsCount > 1) {
        path = fragments.slice(1).join(propertySeparator);
...
function _getArrayItem(indexSeparator, target, path) {
    ...
    if (fragmentsCount > 1) {
        path = fragments.slice(1).join(indexSeparator);
        item = _getArrayItem(indexSeparator, item, path);
    }
...
function _setArrayItem(indexSeparator, target, path, value) {
    ...
    if (fragmentsCount > 1) {
        ...
        path = fragments.slice(1).join(indexSeparator);
        result = _setArrayItem(indexSeparator, item, path, value);
    } else {
        target[index] = value;
        result = true;
    }
