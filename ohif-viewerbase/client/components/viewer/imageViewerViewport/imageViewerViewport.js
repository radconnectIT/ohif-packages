...
const loadDisplaySetIntoViewport = (data, templateData) => {
    ...
    let PromiseManager;
    try {
        // Get the image loader promises (Viwerbase.imageLoaderPromises)
        PromiseManager = OHIF.viewerbase.imageLoaderPromises(imageId);
    } catch (error) {
        console.log(error);
        if (!imagePromises) {
    ...
    // loading image configurable callbacks
    const callbacks = imageViewerViewportData.callbacks;

    // Check if it has before loading image callback
    if (typeof callbacks.before === 'function') {
        OHIF.log.info('imageViewerViewport before loading image callback');
        callbacks.before(PromiseManager, templateData);
    }

    // Start loading the image.
     PromiseManager.all().then(results => {
        let enabledElement;
        try {
        ...
        // Check if it has promises resolved callback
        if (typeof callbacks.promisesResolved === 'function') {
            OHIF.log.info('imageViewerViewport right after promises resolved callback');
            callbacks.promisesResolved(PromiseManager, results);
        }

        // Get the image promise. Check imageLoaderPromises for more info
        const image = results[PromiseManager.getPromiseIndexByName('image')];

        if (!image) {
            throw new OHIF.base.OHIFError('loadDisplaySetIntoViewport invalid image promise');
        }
        ...
        // Check if it has after display image callback
        if (typeof callbacks.imageDisplayed === 'function') {
            OHIF.log.info('imageViewerViewport right image is displayed callback');
            callbacks.imageDisplayed(element);
        }
        ...
        const toolManager = OHIF.viewer.toolManager;
        ...
        const onImageRendered = (event, eventData) => {
            ...
            // Check if it has onImageRendered image loader callback
        ...
        const onNewImage = (event, eventData) => {
            ...
            // Check if it has onNewImage image loader callback
        ...
        if (imagePlane && imagePlane.frameOfReferenceUID) {
            ...
            if (referenceLines && referenceLines.isEnabled()) {
                // Enable reference lines on the current element (i.e., make it a target for source elements)
                referenceLines.addTarget(element);
            }
        ...
        layoutManager.updateSession();
         ...
         // Check if it has after image loader callback
