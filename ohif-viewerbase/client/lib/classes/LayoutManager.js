...
export class LayoutManager extends EventSource {
    ...
    setActiveViewportByIndex(viewportIndex, windowIndex) {
        ...
        const viewerports = $(VIEWPORT_ELEMENT_SELECTOR);
        if (viewportIndex >= 0 && viewportIndex < viewerports.length) {
            const element = viewerports.get(viewportIndex);
     ...
     */
    resetPreviousLayout() {
        const enlargedElementState = this.getEnabledElementsState();
        ...
        // Defer the execution because the element is not enabled yet
        Meteor.defer(() => {
            const enabledElementsState = this.previousEnabledElementsState.slice(0);

            // Update the element at zoomedViewportIndex by its new state
            // The user can have edited when it was enalarged
            enabledElementsState[this.zoomedViewportIndex] = enlargedElementState[0];
            this.updateViewportElementsState(enabledElementsState);
        });
    }
